...
class Skill(Document):
	...
	def validate_no_cycle(self):
		...
		while current:
			if current in seen:
				frappe.throw(_("Circular parent_skill reference detected involving '{0}'").format(current))
			seen.add(current)
	...
	def validate_description(self):
		...
		if len(self.description) > MAX_DESCRIPTION_LENGTH:
			frappe.throw(_("Description must be {0} characters or fewer").format(MAX_DESCRIPTION_LENGTH))
